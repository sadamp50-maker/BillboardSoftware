...
# ---------------- Archive Helpers ----------------
ARCHIVE_TABLE = "billboards_archive"

def initialize_archive_db():
    """Create archive table with same columns if not exists."""
    conn = get_conn()
    cur = conn.cursor()
    cols_sql = ", ".join([f"'{c}' TEXT" for c in COLUMNS])
    cur.execute(f"CREATE TABLE IF NOT EXISTS {ARCHIVE_TABLE} ({cols_sql});")
    conn.commit()
    conn.close()

def move_row_to_archive_by_sno(s_no):
    """
    Move a row identified by 'S No.' to archive table and clear the row in main table.
    This function will:
    1) select the first row with the given S No.
    2) insert its data into archive table
    3) clear that row in the main table (set other columns to empty) to keep S No. entries stable
    """
    conn = get_conn()
    cur = conn.cursor()
    # select the row (first matching)
    cur.execute(f'SELECT rowid, * FROM {TABLE_NAME} WHERE "S No." = ? LIMIT 1', (str(s_no),))
    found = cur.fetchone()
    if not found:
        conn.close()
        return False
    # columns in result: rowid, then each COLUMNS
    rowid = found[0]
    values = list(found[1:1+len(COLUMNS)])
    # insert into archive
    placeholders = ", ".join(["?"] * len(COLUMNS))
    cur.execute(f"INSERT INTO {ARCHIVE_TABLE} ({', '.join(["\""+c+"\"" for c in COLUMNS])}) VALUES ({placeholders})", tuple([str(v) for v in values]))
    # clear the row in main table except S No.
    set_parts = []
    vals = []
    for c in COLUMNS:
        if c == "S No.":
...
def export_db_with_archive_to_excel_bytes():
    """
    Export both main table and archive table into a single Excel file with two sheets.
    """
    conn = get_conn()
    main_df = pd.read_sql_query(f"SELECT * FROM {TABLE_NAME}", conn)
    try:
        arch_df = pd.read_sql_query(f"SELECT * FROM {ARCHIVE_TABLE}", conn)
    except Exception:
        arch_df = pd.DataFrame(columns=COLUMNS)
    conn.close()
    output = BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        main_df.to_excel(writer, index=False, sheet_name="Billboards")
        arch_df.to_excel(writer, index=False, sheet_name="Archive")
    return output.getvalue()
...
if selected:
    ...
    if not full_row.empty:
        ...
        # ---------------- Move to Archive ----------------
        if st.sidebar.button("📦 Move to Archive", key=f"move_{sno}"):
            moved = move_row_to_archive_by_sno(sno)
            if moved:
                st.sidebar.success(f"Row S No. {sno} moved to Archive and cleared from main.")
                # refresh main display variables
                df_raw = load_df_from_db()
                df_raw["_rowid_"] = df_raw["_rowid_"].astype(int)
                display_df = df_raw.drop(columns=["_rowid_"]).copy()
                df_filtered = display_df.copy()
                ...
                st.sidebar.warning("Move failed: row not found.")
        # -------------------------------------------------
...
# Export Excel with two sheets: Billboards + Archive
combined_excel = export_db_with_archive_to_excel_bytes()
st.download_button("⬇️ Download Excel (Billboards + Archive)", data=combined_excel, file_name="Billboard_DB_with_Archive.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
# Also provide separate CSV downloads if wanted
conn = get_conn()
full_df = pd.read_sql_query(f"SELECT * FROM {TABLE_NAME}", conn)
try:
    arch_df = pd.read_sql_query(f"SELECT * FROM {ARCHIVE_TABLE}", conn)
...
st.download_button(
    ...
    file_name="Billboard_Main.csv",
    mime="text/csv"
)
st.download_button(
    "⬇️ Download Archive CSV",
    data=arch_df.to_csv(index=False).encode('utf-8'),
    file_name="Billboard_Archive.csv",
    mime="text/csv"
)
st.markdown("""
<style>
